...
possible_actions = possible_actions = np.identity(
    env.action_space.n, dtype=int).tolist()

# MODEL HYPERPARAMETERS
# Our input is a stack of 4 frames hence 160x130x4 (Width, height, channels)
state_size = [160, 130, 4]
action_size = env.action_space.n           # n possible actions
...
def preprocess_frame(frame):
    ...
    # Resize
    preprocessed_frame = transform.resize(normalized_frame, [160, 130])

    return preprocessed_frame  # 160x130x1 frame


stack_size = 4  # We stack 4 frames

# Initialize deque with zero-images one array for each image
stacked_frames = deque([np.zeros((160, 130), dtype=np.int)
                        for i in range(stack_size)], maxlen=4)


def stack_frames(stacked_frams, screen, is_new_episode):
    # Preprocess frame
    frame = preprocess_frame(screen)

    if is_new_episode:
        # Clear our stacked_frams
        stacked_frams = deque([np.zeros((160, 130), dtype=np.int)
                               for i in range(stack_size)], maxlen=4)
        ...
        # Stack the frames
        stacked_state = np.stack(stacked_frams, axis=2)
...
TargetNetwork = DDDQNNet(state_size, action_size,
...
if gen_mem:
    ...
    for i in tqdm(range(pretrain_length)):
        ...
        p = [0 for x in range(env.action_space.n)]
        p[action] = 1
        action = np.array(p)
        ...
        if done:
            ...
            state = next_state
        if i % 5000 == 0:
            with open('memory_{}.mem'.format(i/5000), 'wb') as f:
                pickle.dump(mem, f)
                f.close()
            mem = Memory(5000)
    env.close()
            ...
            for step in range(max_steps):
                if step % 500 == 0:
                    if memory is not None:
                        with open('memory_{}.mem'.format(mem_index), 'wb') as f:
                            pickle.dump(memory, f)
                    mem_index = random.randrange(1, 20)
                    with open('memory_{}.mem'.format(mem_index), 'rb') as f:
                ...
                # Do the action
                next_state, reward, done, info = env.step(action.index(1))
                episode_rewards.append(reward)
                # If the game is finished
                if done:
                    # the episode ends so no next state
                    next_state = np.zeros(ori_state.shape, dtype=np.int)
                    next_state, stacked_frames = stack_frames(
                        stacked_frames, next_state, False)
