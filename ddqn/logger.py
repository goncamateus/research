...
class Logger:
    ...
    def __init__(self, header, directory_path):
        ...
        self.score = Stat("run", "score", RUN_UPDATE_FREQUENCY,
                          directory_path, header)
        self.step = Stat("run", "step", RUN_UPDATE_FREQUENCY,
                         directory_path, header)
        self.loss = Stat("update", "loss",
                         TRAINING_UPDATE_FREQUENCY, directory_path, header)
        self.accuracy = Stat("update", "accuracy",
                             TRAINING_UPDATE_FREQUENCY, directory_path, header)
        self.q = Stat("update", "q", TRAINING_UPDATE_FREQUENCY,
                      directory_path, header)
...
class Stat:
    ...
    def add_entry(self, value):
        ...
        if len(self.values) % self.update_frequency == 0:
            ...
            print(self.y_label + ": (min: " + str(min(self.values)) +
                  ", avg: " + str(mean_value) + ", max: " + str(max(self.values)))
            print('{"metric": "' + self.y_label +
                  '", "value": {}}}'.format(mean_value))
            self._save_csv(self.directory_path +
                           self.y_label + ".csv", mean_value)
