...
def print_real(state, num_mates, num_ops):
    new = remake_state(state, num_mates, num_ops, True)
...
def main():
    ...
    if args.record:
        ...
        hfo_env.connectToServer(hfo.HIGH_LEVEL_FEATURE_SET,
                                'formations-dt', args.port,
                                'localhost', 'base_left', False)
    # To get low level features instead:
...
# ----------------------------------------------------CONECTION TO SERVER-----------------------------------------
    ...
    for episode in itertools.count():
        ...
        while status == hfo.IN_GAME:
            ...
            if ticks == 15:
                print_real(state, num_teammates, num_opponents)
            ticks += 1
